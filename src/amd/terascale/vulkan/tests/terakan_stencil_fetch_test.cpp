...
main()
{
   ...
   /* Sampling needs a stencil-only view, but the attachment must cover both aspects of the
    * combined image, so they cannot be the same view.
    */
   ...
   VK_CHECK(vkCreateImageView(device, &attachment_view_info, nullptr, &attachment_view));
   ...
   vkDestroyImageView(device, attachment_view, nullptr);
