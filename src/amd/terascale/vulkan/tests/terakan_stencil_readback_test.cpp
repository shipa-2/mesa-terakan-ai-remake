...
main()
{
   VkApplicationInfo const application_info = {
      ...
      .pApplicationName = "terakan-stencil-readback-test",
   ...
   for (uint32_t y = 0; y < kHeight; ++y) {
      for (uint32_t x = 0; x < kWidth; ++x) {
         uint32_t const actual = readback_mapping[y * kWidth + x];
         if (actual != kClearStencil) {
            if (mismatches < 8) {
               std::fprintf(stderr, "stencil(%u,%u) = 0x%02X, expected 0x%02X FAIL\n", x, y,
                            actual, kClearStencil);
            }
            ++mismatches;
         }
      }
   }
   std::printf("stencil_readback texels=%u mismatches=%u %s\n", kWidth * kHeight, mismatches,
