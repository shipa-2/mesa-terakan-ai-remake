...
/* The reducing stencil resolve modes, VK_RESOLVE_MODE_MIN_BIT and VK_RESOLVE_MODE_MAX_BIT.
 *
 * The stencil programs share their layout with the depth ones covered by terakan_resolve_modes and
 * differ only in the reducing opcode (an unsigned integer comparison rather than a floating-point
 * one) and the export slot DB reads the aspect from, so this test exists mainly to give them their
 * own readback proof rather than to explore new ground.
 *
 * Each sample of a multisample attachment is given a different stencil value, by restricting every
 * draw to one sample through the pipeline's sample mask and writing through STENCIL_REPLACE with a
 * static per-pipeline reference, and the three modes are then resolved out of that same attachment
 * into three destinations. The values are chosen so that the minimum, the maximum and sample zero
 * are three distinct values at four and eight samples, which is what makes a mode that silently
 * behaves like another one visible.
 *
 * The same attachment is resolved three times: the first render draws and resolves sample zero, and
 * the two that follow load it back untouched and resolve the reducing modes out of it.
 */
 ...
 */
constexpr uint8_t kSampleStencils[kMaxSampleCount] = {0x40, 0x20, 0x80, 0xC0,
                                                      0x60, 0xA0, 0x30, 0xF0};
...
uint8_t
expected_stencil(VkResolveModeFlagBits mode, uint32_t sample_count)
{
   uint8_t result = kSampleStencils[0];
   for (uint32_t sample = 1; sample < sample_count; ++sample) {
      if (mode == VK_RESOLVE_MODE_MIN_BIT && kSampleStencils[sample] < result)
         result = kSampleStencils[sample];
      else if (mode == VK_RESOLVE_MODE_MAX_BIT && kSampleStencils[sample] > result)
         result = kSampleStencils[sample];
   }
   return result;
}
...
VkResult
create_stencil_image(VkDevice device, VkPhysicalDevice physical_device, VkSampleCountFlagBits samples,
                   VkImageUsageFlags usage, Image & out)
{
   ...
   VkResult const view_result = vkCreateImageView(device, &view_info, nullptr, &out.view);
   if (view_result != VK_SUCCESS)
      return view_result;
   view_info.subresourceRange.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT;
   return vkCreateImageView(device, &view_info, nullptr, &out.depth_view);
}
...
main(int argc, char ** argv)
{
   ...
   VkApplicationInfo const application_info = {
      ...
      .pApplicationName = "terakan-stencil-resolve-modes-test",
   ...
   if ((resolve_properties.supportedStencilResolveModes & wanted) != wanted) {
      std::fprintf(stderr, "supportedStencilResolveModes is 0x%x, expected 0x%x to be present\n",
                   resolve_properties.supportedStencilResolveModes, wanted);
      return 1;
   }
   ...
   Image multisample;
   VK_CHECK(create_stencil_image(device, physical_device, kSamples,
                               VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
                                  VK_IMAGE_USAGE_SAMPLED_BIT,
                               multisample));
   Image destinations[kModeCount];
   for (Image & destination : destinations) {
      VK_CHECK(create_stencil_image(device, physical_device, VK_SAMPLE_COUNT_1_BIT,
                                  VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
   ...
   /* Not a value any of kSampleStencils reaches, so a resolve that leaves a texel untouched is
    * still caught.
    */
   std::memset(readback_mapping, 0xFF, image_bytes * kModeCount);
   ...
   /* One pipeline per sample: the sample mask confines its draw to that sample, and a static
    * per-pipeline stencil reference gives that sample its value. STENCIL_REPLACE always writes the
    * reference regardless of the stencil test result, so the comparison itself does not matter.
    */
   ...
   for (uint32_t sample = 0; sample < kSampleCount; ++sample) {
      ...
      VkStencilOpState const stencil_op = {
         ...
         .compareMask = 0xFF,
         .writeMask = 0xFF,
         .reference = kSampleStencils[sample],
      };
   ...
   VkImageSubresourceRange const whole_stencil = {VK_IMAGE_ASPECT_STENCIL_BIT, 0, 1, 0, 1};
   auto const barrier = [&](VkImage image, VkAccessFlags src_access, VkAccessFlags dst_access) {
      VkImageMemoryBarrier const image_barrier = {
         ...
         .image = image,
         .subresourceRange = whole_stencil,
      };
   ...
   for (uint32_t mode_index = 0; mode_index < kModeCount; ++mode_index) {
      ...
      /* A depth attachment of the multisample image's depth view has to be bound alongside, see the
       * comment on Image::depth_view. It resolves nowhere: only the stencil aspect is under test.
       */
      VkRenderingAttachmentInfoKHR const depth_attachment = {
         .sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR,
         .imageView = multisample.depth_view,
         .imageLayout = VK_IMAGE_LAYOUT_GENERAL,
   ...
   for (uint32_t mode_index = 0; mode_index < kModeCount; ++mode_index) {
      ModeCase const & mode_case = kModes[mode_index];
      uint8_t const expected = expected_stencil(mode_case.mode, kSampleCount);
      uint8_t const * const pixels = readback_mapping + kWidth * kHeight * mode_index;
      uint32_t mismatches = 0;
      uint8_t first_bad = 0;
      for (uint32_t texel = 0; texel < kWidth * kHeight; ++texel) {
         if (pixels[texel] == expected)
            continue;
         if (mismatches == 0)
      ...
      int seen_sample = -1;
      for (uint32_t sample = 0; sample < kSampleCount; ++sample) {
         if (first_bad == kSampleStencils[sample])
            seen_sample = static_cast<int>(sample);
      }
      std::fprintf(stderr, "%s: %u/%u texels wrong, first is 0x%02x, expected 0x%02x%s",
                   mode_case.name, mismatches, kWidth * kHeight, first_bad, expected,
                   seen_sample >= 0 ? "" : "\n");
      if (seen_sample >= 0)
         std::fprintf(stderr, ", which is sample %d\n", seen_sample);
      ++failures;
   }
   std::printf("stencil_resolve_modes samples=%u modes=%u bad=%u %s\n", kSampleCount, kModeCount,
   ...
   vkDestroyImageView(device, multisample.depth_view, nullptr);
   ...
   for (Image const & destination : destinations) {
      ...
      vkDestroyImageView(device, destination.depth_view, nullptr);
