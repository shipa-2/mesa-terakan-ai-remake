 ...
 *
 * It does not reproduce, and that is the result rather than a failure of the tool. The sequence
...
int main(void){
   ...
   for(unsigned c=0;c<sizeof(cases)/sizeof(*cases);++c){
      ...
      /* dEQP gives the image a full mip chain and clears only level zero, so the surface has the
       * whole chain's layout while the clear touches one level of it.
       */
      uint32_t levels=1; for(uint32_t e=w>h?(w>d?w:d):(h>d?h:d); e>1; e>>=1) ++levels;
      ...
      VkDeviceMemory imem; CK(vkAllocateMemory(dev,&mai,NULL,&imem)); CK(vkBindImageMemory(dev,img,imem,0));
      ...
      if(wrong==0){
         printf("%-10s %ux%ux%u: every texel cleared\n",cases[c].name,w,h,d);
      } else {
