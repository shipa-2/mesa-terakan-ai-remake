...
| Correct meta blit format coverage | 5/5 | 3/5 | Now measured rather than assumed, with the CTS binary that turned out to be available. `blit_image.simple_tests` went from the previously recorded 58 passing and 56 failing to **114 passing and none failing**, confirming the 3D blit fix held. The per-format matrix was sampled deterministically (every 43rd of `blit_image.all_formats`): **415 passed, 204 failed**. The failures were systematic, not scattered -- **every `*_USCALED` and `*_SSCALED` format failed every case it ran**, twenty formats with 145 failures and not one pass between them, while advertised as blittable. A second systematic cause hid behind the rest: split by image type, 3D blits were failing 43 of 44 while looking like a per-format tail in aggregate, because the loop over scaled regions used `copies[0]` as scratch space and overwrote the first region the blit had already converted to a copy. Both are fixed. Two further withdrawals followed, each measured: the signed 2_10_10_10 formats from the texture path, and linear filtering from unpacked three-component formats -- every remaining blit failure at that point used `VK_FILTER_LINEAR` while nearest failed nothing across 220 cases. **The sample now stands at 18 failures from 204, and both remaining causes are identified**: 12 are three-component SCALED copies through the 3x-expansion path, and 6 are 3D linear blits, where `terakan_meta_blit_depth_source_slice` documents that the source is sampled as a 2D array with no depth filter, so the depth axis takes the nearest slice whatever was asked for. Both want the hand-written meta shader work. `terakan_blit_mixed_regions` covers the region handling. The other 59 are thin: `r32g32b32_sfloat` 5 of 11, two `a2*_snorm_pack32` formats a few each, the rest single cases. SCALED image support is now withdrawn -- sampled, blit and storage, keeping vertex and texel buffers, matching what `radv_is_sampler_format_supported` does -- which takes the sample to 415 passing and 69 failing with nothing that passed before now skipped. The 69 that remain are thin and per-format (`r32g32b32_sfloat` 5 of 11, the two `a2*_snorm_pack32` a few each, the rest single cases). Withdrawing also surfaced 12 previously-passing `image_to_image` failures, all three-component SCALED formats copied through the 3x-expansion path -- unexplained and not yet attributed to either the driver or a stricter CTS verification path | All basic CTS blits pass for RGBA, BGRA, R32, reversed source/destination axes and 3D slices |
...
| Wire the TeraScale 1 register-emission helpers into command buffer recording, keep surveying and porting per-draw CB/DB state, and build command stream submission | 5/5 | 5/5 | Minimal logical-device creation is now enabled only for hardware-validated R700: physical-device ISA selection, conservative maximum BO alignment and internal allocations are derived from the actual chip family and DRM-reported tiling/backend topology. R600 remains refused. The begin-command-buffer atom is wired to the separate TeraScale 1 SQ/default-register writers. R700 `CB_COLOR_CONTROL` now repacks the proven `DISABLE`/`NORMAL`/`RESOLVE_BOX`, ROP3, degamma and per-target blend-enable fields rather than passing Evergreen `MODE` through the same address; its `CB_BLENDn_CONTROL` packets strip Evergreen-only enable bit 30 and move those enables into `TARGET_BLEND_ENABLE`, following `r600_create_blend_state_mode()`. R700 `DB_ALPHA_TO_MASK` is emitted at its real `0x028D44` offset with the classic driver's regular `2,2,2,2` sample offsets, not Evergreen's potentially dithered value at `0x028B70`. `MULTIWRITE_ENABLE` is deliberately still clear pending the shader-export/MRT survey, and Evergreen eliminate/decompress modes are safely disabled because no exact classic mapping has been established. `DB_SHADER_CONTROL` still needs translation: bits through `DUAL_EXPORT_ENABLE` are shared, but Evergreen-only source-format/execution/depth-order fields cannot pass through, and R700 places conservative-Z export in `DB_RENDER_CONTROL`. Queue submission still returns `VK_ERROR_DEVICE_LOST` before the winsys because the remaining per-draw packets and meta shaders are not hardware-validated. Real RV710 `1002:954f` passes create/destroy and the guarded-submit negative check; the new CB/DB control work is packet-tested only, not submitted. | A trivial graphics submission completes on R700 with register and memory readback checks |
