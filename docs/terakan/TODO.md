...
| Enforce robust buffer and image bounds everywhere | 5/5 | 4/5 | Both dynamic-descriptor paths are done and regression-covered: the SIZE reclamping of the `resource[1]` read path (`terakan_dynamic_offset_bounds`), and the UAV/colour path, where `BASE`/`VIEW`/`DIM` are now rebuilt together for the dynamic offset instead of `BASE` being moved on its own (`terakan_dynamic_uav_bounds`, five offsets against a poisoned guard region). What is left is image bounds, not buffer bounds | Guard regions remain intact for misaligned, dynamic and end-of-range accesses |
